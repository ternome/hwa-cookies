...
| **type-2** | “We use cookies to ensure the proper functioning of our Project and to enhance its performance, and we also may transfer the data about your usage of the Project to our partners in ads and analytics.” (редизайн 13.07.2026 по прод-скрину; Frame 1714 устарел) | `Learn more about cookies` (US-модалка) · **`Accept all`** | opt-out-режимы: US (CPRA-штаты) и т.п. |
...
· `?geo=DE` · `?cb_reset=1` (сброс сохранённого выбора) · `?cb_gift=0` (вариант без подарка).
...
3. ✅ Решено (08.07.2026): для type-3 «Cookie Settings» открывает модалку «Learn more
   ...
   в consent добавлена категория `partners` (только type-3). Для type-2 (13.07.2026):
   кнопка «Learn more about cookies» открывает US-вариант окна — секция «For US»,
   opt-out-текст, без тумблеров (кроме disabled Operational), один «Accept»;
   компактная панель тогглов удалена как неиспользуемая.
